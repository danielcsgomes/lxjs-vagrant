...
Vagrant.configure("2") do |config|
    ...
    if Dir.glob("#{File.dirname(__FILE__)}/.vagrant/machines/default/*/id").empty?
        ...
        # Install nodejs
        pkg_cmd <<  "cd /tmp;wget http://nodejs.org/dist/v0.10.18/node-v0.10.18-linux-x64.tar.gz;"
        ...
        # Install couchdb
        pkg_cmd <<  "sudo adduser --quiet --disabled-login --disabled-password --no-create-home --gecos \"\" couchdb;"
        pkg_cmd <<  "sudo apt-get install -y build-essential erlang-base-hipe erlang-dev erlang-manpages erlang-eunit erlang-nox libicu-dev libmozjs-dev libcurl4-openssl-dev pkg-config links redis-server;"
        pkg_cmd <<  "sudo apt-get install -y libmozjs185-dev libicu-dev libcurl4-gnutls-dev libtool;"
        pkg_cmd <<  "cd /tmp;wget http://mirrors.fe.up.pt/pub/apache/couchdb/source/1.4.0/apache-couchdb-1.4.0.tar.gz;tar -xvf apache-couchdb-1.4.0.tar.gz;rm apache-couchdb-1.4.0.tar.gz;"
