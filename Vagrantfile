...
Vagrant.configure("2") do |config|
    ...
    if Dir.glob("#{File.dirname(__FILE__)}/.vagrant/machines/default/*/id").empty?
        ...
        # Update OS
        pkg_cmd =  "sudo apt-get update -qq -y -f;"
        ...
        pkg_cmd <<  "sudo apt-get install -qq -f -y build-essential erlang-base-hipe erlang-dev erlang-manpages erlang-eunit erlang-nox libicu-dev libmozjs-dev libcurl4-openssl-dev pkg-config links redis-server;"
        pkg_cmd <<  "sudo apt-get install -qq -f -y libmozjs185-dev libicu-dev libcurl4-gnutls-dev libtool vim;"
        ...
        pkg_cmd <<  "cd apache-couchdb-1.4.0;sudo ./configure;make && sudo make install;"
        ...
        pkg_cmd << "apt-get update -qq; apt-get install -qq -y linux-image-generic-lts-raring; "
